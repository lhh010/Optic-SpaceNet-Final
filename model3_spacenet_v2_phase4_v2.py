"""
================================================================================
 模型三 Phase4 v2 (SpaceNet V2 KD): KD + Conv+Linear 全 int4 QAT + STE
================================================================================
 修复 (vs 旧版 model3_spacenet_v2_phase4.py):
   - 旧版: optic_qat_v2 + first_layer_fp32 bug → Conv 层 QAT 全关 → int4=78%
   - 新版: optic_qat_v3 + KD + Conv+Linear 全 int4 QAT
   - 预期 int4 精度: 87-91% (大幅优于旧版 78%)

 量化策略:
   - 所有 Conv2d: int4 QAT (STE + 噪声注入), bias=False
   - 所有 Linear:  int4 QAT (STE + 噪声注入), bias=False
   - BN: float32 保留
   - 教师: ResNet-18 (fp32), T=4.0, α=0.7

 用法:
   python model3_spacenet_v2_phase4_v2.py
================================================================================
"""
...
from optic_qat_v3 import (
    ...
    evaluate_model_v3, print_alignment_detail, set_quant_lr,
)
...
# ============================================================
#  学生模型: SpaceNet — 全 int4, bias=False
# ============================================================
...
class KDPhase4Trainer:
    """KD + Phase4: 教师(fp32) → 学生(Conv+Linear 全 int4, bias=False)"""
    ...
    def run(self):
        ...
        # === Step 1: 转换学生 (Conv+Linear 全 int4 QAT) ===
        ...
        prepare_model_v3(
            ...
            quantize_linear=True,  # ★ Phase4: Linear 也 int4
            preserve_bn=True,
        )
        ...
        print(f"  int4 QAT Conv: {qc}, int4 QAT Linear: {ql}, BN: {bn} (float32)")

        alignment = print_alignment_detail(student, "Student (Conv+Linear int4)")
        ...
        print(f"\n[Step 2] KD+Phase4 训练 ({epochs} epochs, T={T}, α={alpha})")
        print(f"  教师: ResNet-18 (fp32) → 学生: Conv+Linear int4, bias=False")
        ...
        result_qat = evaluate_model_v3(student, self.val_loader, self.device,
                                       nn.CrossEntropyLoss())
        print(f"  Int4 模式 (全光计算) 准确率: {result_qat['accuracy']:.2%}")
        ...
        fname = cfg.get('save_path', 'spacenet_v2_phase4_v2_ste.pth')
        ...
        print(f"  训练完成 — 结果汇总")
        print(f"{'='*60}")
        print(f"  学生模型:          OpticSpaceNet (Conv+Linear int4, bias=False)")
        print(f"  教师模型:          ResNet-18 (fp32)")
...
def main():
    ...
    print(f"设备: {DEVICE}")
    print(f"\n{'='*60}")
    print(f"  Model 3 Phase4 v2: KD + Conv+Linear 全 int4, bias=False")
    ...
    try:
        ...
        print(f"  [ERROR] 教师权重未找到: teacher_resnet18.pth")
    ...
    config = {
        ...
        'temperature': 4.0,
        'alpha': 0.7,
        'model_name': 'OpticSpaceNet Phase4 v2 (KD + Conv+Linear int4)',
        'save_path': 'spacenet_v2_phase4_v2_ste.pth',
        'fp32_baseline': '91.44% (全 fp32 KD)',
        'device': DEVICE,
        'num_classes': 10,
    }

    trainer = KDPhase4Trainer(teacher, student, train_loader, val_loader, config)
