...
> **一句话总纲（开场/收尾都能用）**：我们把三种 CNN 迁移到曦智 Gazelle 光计算硬件，用"硬件先于模型"的方法论，做到 **光计算占比 ≥ 90%、Model 1 真机 98.15%（q650）、Model 2/3 真机 90%+（全量 5400）、量化无损甚至反超 FP32**。
...
- **Model 1 osimulator q650 扩大抽样**：变体 A **98.15%**（反超 FP32 +0.98%）· 变体 B **97.54%**（超 FP32 +0.37%），n=650 SE≈1.9%，光计算占比 A 97.74% / B 73.64%
...
**口述讲稿**
> "这是精度结果。三个模型在干净验证集上 int8 精度分别达到 98%、92%、92%，全部超过目标，而且量化几乎无损——Model 2 甚至比 FP32 基准还高近两个点。更重要的是真机验证：Model 2 在 osimulator 上跑完全部 5400 张达到 90.43%，反超了原始 FP32 模型。Model 1 因为算力太大无法跑全量，我们扩大抽样到 650 张——变体 A 达到 98.15%，比 FP32 基准还高近 1 个点。请注意，这里所有数字都是我们主动修复数据泄漏 bug 之后、干净重训复测的真值。这说明我们的量化模型在真实光子硬件上是真正可用的。"
...
| 真机精度     | 98.15%（变体A, q650） | **90.43%**（全量5400） | **90.28%**（全量5400） |
| 参数量      | 2.39M             | 268K               | 268K               |
| 总 MOPs/张 | 156.6M            | 1.05M              | 1.05M              |
...
**口述讲稿**
> "总结一下：我们把三种 CNN 全部迁移到光计算，光计算占比超过 90%，Model 1 真机 98.15%、Model 2/3 真机 90% 以上，量化几乎无损。进阶方面，我们实现了 int8 全流程真机部署、用噪声匹配训练做到近无损真机推理、并完成了三模型多量化的完整消融。方法论上始终坚持'硬件先于模型'。下一步我们计划做光电混合加速——已经搭好了三种切分方案的脚本——并推动真实卫星在轨验证。以上是我们的汇报，感谢各位评委，请提问。"
...
`docs/figures/compute_vs_accuracy_final.png` 当前的 osim 点需要更新为最新数据：
- **Model 1**: q50 抽样 98.00%/100.00% → **q650 抽样 98.15%（变体 A）/ 97.54%（变体 B）**（2026-07-18 新增）
- **Model 2**: q500 抽样 89.00% → **全量 5400 真值 90.43%**
- **Model 3**: q500 抽样 90.80% → **全量 5400 真值 90.28%**
**修复（改 `src/scripts/plot_compute_vs_accuracy.py`）**：
```python
# Model 1 变体 A: 'y_osim': 98.00  →  98.15
'y_osim': 98.15,
...
'y_osim': 90.28,
```
并把页脚 `"Model 1 n=50 / Model 2/3 n=500"` 改为 `"Model 1 n=650 / Model 2/3 n=5400 (full test set)"`，重跑 `python src/scripts/plot_compute_vs_accuracy.py`。
...
|---|---|---|---|
| Model 1（A） | 97.17 | 97.87 | 98.15（变体A, q650）|
...
osim  = [98.15, 90.43, 90.28]
osim_note = ['(q650)', '(全量5400)', '(全量5400)']
...
ax.annotate('M1 真机反超 FP32 +0.98%', xy=(0+w, 98.15), xytext=(-0.25, 99.0),
            fontsize=9, color='#0072B2', arrowprops=dict(arrowstyle='->', color='#0072B2'))
