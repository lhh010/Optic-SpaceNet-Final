...
| 判据 | 方式 | 耗时 |
|---|---|---|
| ② error_std < ±2% | 自动：16 组已知探针矩阵乘 vs numpy 精确参考 | 秒级 |
| ③ canary gap < 0.5pt | 自动：10 图 mini-run 的 hw acc vs 干净参考 acc 差（EuroSAT-10 等价演示，页面如实标注；真机判据为 MNIST canary） | ~1–2 min |
| ④ mini-run 对齐 | 自动：同一次 run 的逐图一致率 + 精度正常性 | （与③共用） |
| ① EBR ≥ 8 | **手动**：板端控制台跑 `compass_evb_test` 读数，页面输入框录入校验 | 板端另计 |

- 总真机占用 ≈ 探针 + 10 图 × 5 光算层往返，**2 分钟以内**；需要更慢/更快调 `HW_CHECK_N`（默认 10）；
- 建议演示顺序：先点「运行②③④」→ 期间口头讲判据设计 → 板端读 EBR 录入 → 四灯全绿再进单图推理演示，叙事正好是「先判据后跑批」的 SOP 还原。
