...
| TIA 噪声 σ  | ≈ 5.34×10⁻⁴（MSE 2.85×10⁻⁷） | 训练噪声匹配项（探测噪声）                  |
...
**硬件匹配噪声**：训练时向权重注入与 Gazelle DAC ENOB=7.5 等量的噪声（`noise_std_ratio = 0.0016`，经硬件标定换算，替代经验主义 0.02×scale 的粗扰动），增强模型对量化舍入与硬件噪声的鲁棒性。
...
| 噪声              | `GazelleNoiseInjector`（DAC ENOB 7.5 + TIA σ=5.34e-4）    |
...
OpticalEngine（自动检测）：
        ...
        ▼
osimulator（COMPASS 8a8w12o，DAC ENOB 7.5 + TIA 物理噪声）
...
| 噪声 | Gazelle 噪声（DAC 7.5 + TIA） | osimulator 物理噪声（DAC 7.5 + TIA σ=5.31） | ✓ |
