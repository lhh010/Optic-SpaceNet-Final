...
卫星在轨遥感计算存在结构性困境：星地下行带宽受限，海量遥感数据必须**在星上完成实时分类**（土地监测、灾害评估、目标识别），而星载平台的功耗、重量、抗辐射能力均严格受限。光计算核心芯片以模拟物理过程完成矩阵乘，天然低功耗、抗单粒子翻转（SEU）、光速并行，与卫星平台约束高度契合；但其外围 DAC/ADC、FPGA 等电气模块仍需常规宇航加固。
