...
================================================================================
 ...
 用法 (在光计算 Docker 容器内):
   python optic_inference_int4.py                        # 默认 Optic 模式全量
   python optic_inference_int4.py --quick 50             # 快速测试
   python optic_inference_int4.py --qat                  # QAT 伪量化交叉验证
...
def print_mops_report(layers, summary):
    ...
    print(f"  [Note] stem 展平=3 对齐率仅 37.5%, int4 下噪声过大, 保留电计算")
    print(f"{'='*110}")
...
def main():
    ...
    print(f"  Training ref: 91.06% int4")
