...
"""
===============================================================================
 Model 5 — J1-RF+ (stem k5 + stage2 3×3) · Gazelle 光计算 QAT 训练脚本
===============================================================================
 设计依据 (eurosat_research/docs R6/R7/R8 + round5_c2_drift_robust.md):
   - 架构: J1 家族 (channels [16,32,64,128], fast_downsample, MaxPool, GAP head)
     + stem k5 (R6: 底层纹理滤波器质量, +0.52pt @ 最高 MACs 效率)
     + stage2 3×3 (R6/R8: RF 17→49px, 160ep 96.93, 长训持续涨)
   - MACs: 5.31M | params: 100K (光计算第一性: 权重存光阵列近免费, 激活计算受限)
   - QAT: v5 干净阶段 (w8/a8/o12, 输出噪声 r=0.0392) → v8 漂移鲁棒阶段
     (probe 实测组分噪声: per-column 偏移/增益 + δW + iid260, 1.5× 余量)

 详细日志: 每 epoch 打印 + runs/<name>_<phase>/metrics.jsonl (结构化)
 用法:
   python train_m5_j1rf_stem5.py                      # 干净阶段 160ep
   python train_m5_j1rf_stem5.py --phase v8 --init <clean best.pth>   # 漂移鲁棒 60ep
   python train_m5_j1rf_stem5.py --epochs 2           # 冒烟测试
===============================================================================
"""
import os
import sys
...
ARCH = dict(
    ...
    kernels=[1, 3, 1],        # stage1 1×1 | stage2 3×3 (RF 49px) | stage3 1×1
    stem_kernel=5,            # 底层纹理滤波器
    head_dims=[128],
    bias=False,
)

# ---- v8 漂移鲁棒噪声参数 (c3d 冠军配方, probe 实测 × 1.5 余量) ----
# 注: stage2 3×3 为 Model 5 新层, col_off/col_gain 按输出通道复用 (同一物理列),
#     dw_rms 用 J1 同位置估计 + 1.5× 余量, 上板前需 probe 复核
V8_NOISE = dict(
...
def main():
    ...
    print("=" * 78)
    print(f"  Model 5 (J1-RF+): stem k5 + stage2 3×3 | 光计算 QAT | phase={phase}")
