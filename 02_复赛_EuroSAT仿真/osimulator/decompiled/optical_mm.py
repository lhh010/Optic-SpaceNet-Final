...
# File: optical_mm.cpython-39.pyc (Python 3.9)
...
class oMAC_mm(torch.nn.Module):
    ...
    def forward(self, input_x, weights, device_mode = ('COMPASS',)):
        ...
        if omac_flag:
            ...
            return None
        output_tensor = torch.mm(input_x, weights)
        return output_tensor

    
    def omm(self, input_x, weights, device_mode = ('COMPASS',)):
        t0 = time.time()
...
if __name__ == '__main__':
    np.random.seed(100)
    opt_mm = oMAC_mm()
    x = np.random.uniform(-1, 1, (4, 4), **('low', 'high', 'size'))
    print('x = ', x)
    weights = np.random.uniform(-1, 1, (4, 1), **('low', 'high', 'size'))
    torch_res = torch.mm(torch.from_numpy(x), torch.from_numpy(weights))
    print('torch_res = ', torch_res)
    print('torch_res shape = ', torch_res.shape)
    results = opt_mm.omm(x, weights, 'COMPASS', **('device_mode',))
    omac_res = results
