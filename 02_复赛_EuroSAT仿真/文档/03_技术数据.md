...
> **光计算硬件仿真评估**需进入主办方提供的 Docker 容器（见 §4.2）。容器外（如 Windows 本机）`import osimulator` 会失败并回退到 `FakeOpticalEngine`，只能跑 QAT 伪量化（秒级）与 NumPy 路径。
...
| `optic_inference_lsq.py`            | LSQ+ int8       | LSQ 专用路径（monkey-patch 保留学习到的 scale/zp） | LSQ+ 仿真验证                            |
