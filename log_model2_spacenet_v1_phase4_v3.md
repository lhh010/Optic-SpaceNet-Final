...
## osimulator 真机全量 (5400) — Model 2 int8 test 真值 (2026-07-16) ★

`python optic_inference_int8.py 2>&1 | tee log_optic_int8_new.md`

上节 q500=89.00% 为抽样数; 本节为**全量 5400 张独立 test 真值**, 钉死硬件 gap。真实 osimulator (COMPASS `8a8w12o`, DAC ENOB 7.5, 输出 σ=5.31); stem 电计算, 其余 5 层光计算 int8。

| 指标 | 值 | 说明 |
|---|---|---|
| **osim int8 (test, 全量 5400)** | **90.43%** | 27000 engine calls (5 光计算层 × 5400), 5.15e+09 MACs |
| osim int8 (q500 抽样) | 89.00% | 445/500 — 抽样数, 已被全量取代 |
| Int8 (val, 干净重训) | 92.06% | 训练精度 |
| Int8 QAT (test 全量 5400) | 92.20% | 干净泛化数 (无硬件噪声), **同 test 集** |
| FP32 基准 | 90.15% | — |
| **Δ osim vs val** | **−1.63%** | 真实硬件 gap ~1.6 点 |
| Δ osim vs Int8 QAT test (同集) | −1.77% | 纯 osim 物理噪声损失 (干净伪量化 → 真硬件) |
| **Δ osim vs FP32 基准** | **+0.28%** | **真机反超原 FP32 模型** |
| 总耗时 | 13357.5s (~3.7h, ~2.5s/张) | — |
| 光计算占比 | 90.65% | 0.9528M / 1.0511M |

**全量进度曲线** (acc 单调收敛到 90.43%):
```
...
**关键结论:**
- **全量 90.43% 取代 q500 89.00%**: q500 的 89% 恰落在 ~1 SE 低处 (n=500 SE≈1.4%), 全量回升到 90.43%。两值方向一致 (都低于 val), 全量才是可信真值。
- **真机反超 FP32 基准** (+0.28%): 干净光计算硬件上 Model 2 int8 仍超原 FP32 模型 90.15%。
- **硬件 gap ~1.6 点, 与 Model 3 基本相等** (M3 全量 90.28%, Δ vs val −1.55%): 见 `log_optic_kd_new.md`。q500 时 "M2 gap (~3 点) > M3 gap (~1 点)" 是抽样噪声, 全量证伪 → **"KD 提升光计算噪声鲁棒性" 不成立** (M2-vs-M3 直接差 0.15%, z≈0.27 不显著)。
- 硬件 gap 来源: 同 test 集上 Int8 QAT 92.20% → osim 90.43% = −1.77%, 即 DAC ENOB 7.5 + TIA σ=5.31 物理噪声相对干净伪量化的损失; 训练时 Gazelle 噪声配方已吸收大部分。
