...
def main():
    ...
    if "w3" not in quant_params:
        raise FileNotFoundError("Missing `w3_int4_lsq_plus.npy` for 3-layer LSQ+ robustness test.")
    inference_fn = make_inference_fn(w1, w2, quant_params["w3"], quant_params)
    results = run_robustness_test("LSQ+", inference_fn)
    out_path = os.path.join(os.path.dirname(__file__), "../../artifacts/robustness/robustness_test_lsq_plus.npy")
