"""
================================================================================
 optic_inference_h1.py — 光电混合模式 H1: Conv 光计算 / Linear 电计算

 模式 H1:
   光计算 (int8 + osimulator): stage1, stage2, stage3        (3 个 Conv)
   电计算 (fp32):              stem, fc1, fc2                (stem + 2 个 Linear)
   光计算 MOPs 占比:           65.5%   (> 50% ✓ 满足硬约束)

 目的: 把两个耗时 Linear (fc1 ~2s/张) 搬到电计算提速, 同时保住 >50% 光计算。
       (此切分等价于 build_optical_model(convert_linear=False); 这里用独立文件实现,
        不修改任何现有代码。)

 适用模型 (架构相同, 仅权重不同):
   Model 2: spacenet_v1_phase4_v3_int8.pth   (val int8 92.06%)
   Model 3: spacenet_v2_phase4_v3_int8.pth   (val int8 91.83%, osim q500 90.80%)

 用法 (光计算 Docker 容器内):
   python optic_inference_h1.py --quick 100            # M2+M3 各 100 张
   python optic_inference_h1.py --model 2 --quick 100  # 只跑 Model 2
   python optic_inference_h1.py --mops-only            # 只打印 MOPs 占比
================================================================================
"""
import os, sys, time
...
# ==================== 本模式配置 ====================
MODE = "H1"
MODE_DESC = "Conv光/Linear电 (stage1/2/3 光计算, fc1/fc2 电计算)"
# 光计算层的 dotted name (Sequential 内索引: stem.0/stage1.0/.../classifier.1=fc1, classifier.4=fc2)
OPTICAL_DOTTED = {"stage1.0", "stage2.0", "stage3.0"}
WEIGHTS = {
    "2": ("spacenet_v1_phase4_v3_int8.pth", "Model 2 SpaceNet V1 v3 int8"),
    "3": ("spacenet_v2_phase4_v3_int8.pth", "Model 3 SpaceNet V2 KD v3 int8"),
}

# LAYER_SPECS: name, type, C_in, C_out, Kh, Kw, stride, pad, pool, is_optical(本模式)
LAYER_SPECS = [
    ("stem.conv",   "Conv",   3,   8,  1, 1, 1, 0,  None,    False),  # 电 (stem)
    ("stage1.conv", "Conv",   8,  16,  2, 2, 2, 0,  "Max2x2", True),  # 光
    ("stage2.conv", "Conv",  16,  32,  2, 2, 2, 0,  None,    True),   # 光
    ("stage3.conv", "Conv",  32,  16,  1, 1, 1, 0,  None,    True),   # 光
    ("fc1",         "Linear", 1024, 256, 0, 0, 0, 0, None,    False), # 电
    ("fc2",         "Linear", 256,  10, 0, 0, 0, 0, None,    False), # 电
]
