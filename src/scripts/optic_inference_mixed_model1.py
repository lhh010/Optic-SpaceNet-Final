...
================================================================================
 ...
 模型: Model 1 Baseline VGG Mixed (Conv=int4 光计算, Linear=fp32 电计算)
   ...
   权重文件:  weights/baseline_vgg_mixed_ste.pth
...
def load_test_data(batch_size=DEFAULT_BATCH, test_ratio=0.2):
    ...
    # 单一数据源: 与其余推理脚本一致取 held-out test 段 (与 train/val 互斥)。
    # 旧版用 val 段 (indices[:sz]) 当 test, 现统一为 test 段, 见 Bug #11。
...
def main():
    ...
    weight_path = "weights/baseline_vgg_mixed_ste.pth"
