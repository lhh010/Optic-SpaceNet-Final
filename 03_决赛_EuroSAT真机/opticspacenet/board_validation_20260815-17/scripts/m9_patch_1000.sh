#!/bin/bash
# M9 补段 off=1000 (labels 修复后补跑, 2026-08-15)
cd /home/uisrc/j1
...
PREF=probe_m9patch1000_

log(){ echo "[$(date '+%F %T')] $*" >> $LOG; }
state(){ echo "$*" > $STATE; }

T0=$(date +%s)
log "=== [M9-PATCH1000] off=1000 probe+calib start ==="
...
log "=== [M9-PATCH1000] off=1000 calib ${CMIN}min -> run budget ${RMAX}min -> LIMIT=$LIMIT ==="
echo $PW | sudo -S -p X env DS3_WEIGHTS_DIR=$WD DS3_OFFSET=1000 DS3_LIMIT=$LIMIT DS3_BATCH=8 \
  DS3_CALIB=$SCAL DS3_CALIB_COL=$COL DS3_LOGITS_OUT=/home/uisrc/j1/logits_${PREF}_off1000.npy \
  python3 run_ds3_gazelle.py >> $LOG 2>&1
ACC=$(grep -a 'FINAL:' $LOG | tail -1 | grep -aoE '[0-9]+[.][0-9]+' | tail -1)
[ -z "$ACC" ] && ACC=-1
log "=== [M9-PATCH1000] off=1000 acc=$ACC calib=${CMIN}min budget=${RMAX}min limit=$LIMIT ==="
state "M9-PATCH1000 off=1000 acc=$ACC $(date '+%F %T')"
log "=== [M9-PATCH1000] cooldown 360s ==="
sleep 360
echo "M9-PATCH1000 DONE acc=$ACC"
