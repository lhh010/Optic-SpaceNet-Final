# -*- coding: utf-8 -*-
"""plot_perf_vs_macs_doc.py — 文档/无标注版: 宽幅但不含 M10/M9 聚焦箭头与甜点区间标注.
与 plot_perf_vs_macs.py 同数据同布局, 仅去掉三个 annotation 叠加."""
import matplotlib
matplotlib.use("Agg")
import matplotlib.pyplot as plt

plt.rcParams["font.family"] = ["Microsoft YaHei", "SimHei", "PingFang SC", "Hiragino Sans GB", "Heiti TC", "Arial Unicode MS", "sans-serif"]
...
points = [
    ...
    (None,                           0.86, 95.30, "R6 消融 (80ep)", (0, 0)),
    (None,                           1.30, 95.09, "R6 消融 (80ep)", (0, 0)),
    (None,                           1.38, 94.78, "R6 消融 (80ep)", (0, 0)),
    (None,                           1.40, 95.63, "R6 消融 (80ep)", (0, 0)),
    (None,                           1.43, 95.39, "R6 消融 (80ep)", (0, 0)),
    (None,                           1.90, 95.67, "R6 消融 (80ep)", (0, 0)),
    (None,                           2.16, 95.33, "R6 消融 (80ep)", (0, 0)),
    ("R6 rf_stem5",                  2.16, 96.04, "R6 消融 (80ep)", (-72, 6)),
    (None,                           2.69, 95.46, "R6 消融 (80ep)", (0, 0)),
    ...
    ("R7 head256",                   1.40, 96.04, "R7/R8 (160ep)", (-12, -14)),
    ("R7 J1 s43",                    1.38, 96.24, "R7/R8 (160ep)", (-70, -2)),
    (None,                           1.68, 96.02, "R7/R8 (160ep)", (0, 0)),
    (None,                           1.82, 95.76, "R7/R8 (160ep)", (0, 0)),
    (None,                           1.90, 95.96, "R7/R8 (160ep)", (0, 0)),
    ("R8 rf_s2k3 (160ep)",           4.52, 96.93, "R7/R8 (160ep)", (-60, 8)),
    ("R8 w200 (160ep)",              4.62, 96.39, "R7/R8 (160ep)", (8, -8)),
    ("M7 v8",                       0.86, 95.00, "M5-M10 (v8)", (8, -14)),
    ("M6 v8",                       1.38, 95.22, "M5-M10 (v8)", (8, 6)),
    ("M9 v8 ★",                     1.52, 95.87, "M5-M10 (v8)", (-66, -16)),
    ("M8 v8",                       2.16, 96.26, "M5-M10 (v8)", (8, -10)),
    ("M10 v8 ★",                    2.56, 96.76, "M5-M10 (v8)", (8, 10)),
    ("M5 v8",                       5.31, 96.65, "M5-M10 (v8)", (-70, 6)),
]

hw_points = [
    ("M7 hw",  0.86, 81.70, (8, -16)),
    ("M6 hw",  1.38, 92.78, (8, 8)),
    ("M9 hw ★", 1.52, 94.43, (-66, -16)),
    ("M8 hw",  2.16, 87.78, (8, 8)),
    ("M10 hw ★", 2.56, 95.33, (8, -18)),
    ("M5 hw",  5.31, 90.17, (8, 8)),
    ("M4 hw", 17.03, 94.19, (8, -18)),
]
...
for gname, style in groups.items():
    ...
    ax.scatter(xs, ys, label=gname, zorder=3, edgecolors="white", linewidths=0.6, **style)
...
hw_show = [p for p in hw_points if p[2] >= 90]
...
for label, x, y, off in hw_points:
    if y < 90:
        continue
    ax.annotate(label, (x, y), textcoords="offset points", xytext=off,
                fontsize=8, color=groups["真机全量 (5400)"]["color"], zorder=6)

# 2M 预算画成半透明带 (而非单线): log 轴上 1.52 距 2 仅 ~4.5% 全轴宽,
# 单线会让 M9 观感上紧贴线; 带状呈现 "M9 在预算内" 更直观
ax.axvspan(0.8, 2.0, color="gray", alpha=0.10, zorder=1)
ax.axvline(2.0, color="gray", ls="--", lw=1, alpha=0.6)
ax.annotate("≤2M 严格预算", xy=(2.0, 91.4), xytext=(2.5, 91.15), fontsize=8.5, color="gray",
            arrowprops=dict(arrowstyle="-", color="gray", lw=0.7, shrinkA=0, shrinkB=1))
...
ax.set_title("Perf vs MACs — QAT clean 口径 (Ltsimulator-test + auto_research R1-R8, EuroSAT; R6 pool_s1x1 = 85.07% 超出范围未画)")
ax.set_xlim(0.8, 400)
ax.set_ylim(91, 99.3)
...
fig.tight_layout()
import os
out = os.path.join(os.path.dirname(os.path.abspath(__file__)), "plot_perf_vs_macs_noarrow.png")
