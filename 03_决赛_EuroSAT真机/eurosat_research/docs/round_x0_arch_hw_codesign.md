...
日期：2026-08-09 · 状态：**完成**（M9/M10 上板 96.40/94.90，M10 真机 SOTA）· 性质：首个**架构 × 硬件噪声联合设计**轮次
...
### C2 · M9/M10 上板（**完成**，`x0/results/C2_ds3_board.md`）

同一 fresh cali 窗口、四项放行判据全过、col calib、各 1000 样本：

| 模型 | hw col acc | GPU QAT test | FAKE | hw−GPU gap |
|---|---|---|---|---|
| **M10 ds3pool3** | **96.40%** | 96.76 | 97.40 | **−0.36pt** |
| **M9 w075ds3** | **94.90%** | 95.87 | 96.30 | −0.97pt |

**M10 真机 96.40% = 新真机 SOTA**（c3d+col 94.60 → +1.80pt，真机首破 95/96）；M9 同超旧 SOTA（+0.30pt）。M10 的 hw 错误 64% 与 torch FP32 同错（模型固有），硬件净代价仅 9 样本——qat_v8 + 同窗口逐列 calib 对该模型基本闭合真机 gap。部署链路：conv3s2 光计算层（k=9C im2col + m≤2 tiling）一次通过，上板前 FAKE 对拍 corr 0.9999；conv3s2 层 resid_std 约为 1×1 层 2.5-3×（与 k 放大一致）。未跑 scalar ABA 与 s43（收窗保数字优先）；建议跨日复现 M10 一次确认稳定性。
...
| **M9** | **J1 w0.75 + conv3s2 下采样**（≤2M 冠军） | 1.52M | 95.87/95.69（单阶段双 seed） | `m9_j1w075ds3_v8probe15.pth` | **真机 94.90%**（C2，超旧 SOTA） |
| **M10** | **J1 + conv3s2 + stem max3**（总冠军） | 2.56M | 96.76/96.56（单阶段双 seed） | `m10_ds3pool3_v8probe15.pth` | **真机 96.40% = 真机 SOTA** |
