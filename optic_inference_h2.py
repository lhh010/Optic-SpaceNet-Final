"""
================================================================================
 optic_inference_h2.py — 光电混合模式 H2: 去掉最慢的 stage2 (Conv 电计算)

 模式 H2:
   光计算 (int8 + osimulator): stage1, stage3, fc1, fc2
   电计算 (fp32):              stem, stage2          (stage2 是 osim 最慢的层 ~3-7s/张)
   光计算 MOPs 占比:           78.2%   (> 50% ✓ 满足硬约束)

 目的: 把 osim 里最慢的单层 (stage2, MOPs 不大却最耗时) 搬到电计算, 用很少的 MOPs
       换大量时间; 其余 4 层 (含大 MOPs 的 stage1/fc1) 留光计算, 占比仍 78%。
       stage1 单层占 49.9% 总 MOPs → 必须留光计算, 本模式满足 >50%。
 ...
 用法 (光计算 Docker 容器内):
   python optic_inference_h2.py --quick 100            # M2+M3 各 100 张
   python optic_inference_h2.py --model 2 --quick 100  # 只跑 Model 2
   python optic_inference_h2.py --mops-only            # 只打印 MOPs 占比
================================================================================
"""
import os, sys, time
...
# ==================== 本模式配置 ====================
MODE = "H2"
MODE_DESC = "去 stage2 (stage2 电计算, stage1/stage3/fc1/fc2 光计算)"
# 光计算层的 dotted name (classifier.1=fc1, classifier.4=fc2)
OPTICAL_DOTTED = {"stage1.0", "stage3.0", "classifier.1", "classifier.4"}
WEIGHTS = {
...
LAYER_SPECS = [
    ...
    ("fc1",         "Linear", 1024, 256, 0, 0, 0, 0, None,    True),  # 光
